...
impl ErrorBuilder {
    // Starters
    pub fn new(status_code: StatusCode, r#type: GenericErrorType, reason: impl Display) -> ErrorBuilder {
    ...
    pub fn internal(reason: impl Display) -> Self {
        ...
        Self::new(status_code, GenericErrorType::Internal, reason)
    }

    pub fn database(reason: impl Display) -> Self {
        let status_code = StatusCode::INTERNAL_SERVER_ERROR;
        Self::new(status_code, GenericErrorType::Database, reason)
    }

    pub fn custom(reason: impl Display) -> Self {
        let status_code = StatusCode::INTERNAL_SERVER_ERROR;
        Self::new(status_code, GenericErrorType::Custom, reason)
    }
    ...
    pub fn r#type(mut self, r#type: GenericErrorType) -> Self {
    ...
    // Produce ready error
    pub fn finish(self) -> GenericError {
        self.inner
    }
}
...
/// Middleware for converting plain actix errors into JSON ones with GenericError schema
